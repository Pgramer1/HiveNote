...
export function isUniversityEmail(email: string): boolean {
  const universityDomains = [
    'adaniuni.ac.in',
    'yahoo.com',// Allow Yahoo for testing purposes - remove in production
...
export async function sendVerificationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'HiveNote <onboarding@resend.dev>',
      to: email,
      subject: 'Verify your HiveNote account',
      html: `
        <!DOCTYPE html>
        <html>
          <head>
...
export async function sendPasswordResetEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: process.env.EMAIL_FROM || 'HiveNote <onboarding@resend.dev>',
      to: email,
      subject: 'Reset your HiveNote password',
      html: `
        <!DOCTYPE html>
        <html>
          <head>
